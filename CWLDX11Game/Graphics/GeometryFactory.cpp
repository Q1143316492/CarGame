...
void GeometryFactory::CreateCylinderFlicker(Mesh & mesh, float radius, float height, float level)
{
	std::vector<Vertex> v;
...
void GeometryFactory::CreateCylinder(Mesh & mesh, float radius, float height, float level)
{
	...
	for (int i = 0; i < 360; i += offset)
	{
		...
		float nx = radius * cos(MathTools::AngleChange(i + offset));
		float nz = radius * sin(MathTools::AngleChange(i + offset));
		v.push_back(Vertex(nx, height / 2.0F, nz, 0.0F, 1.0F, 0.0F, 1.0F, 0.0F));
		v.push_back(Vertex(x, height / 2.0F, z, 1.0F, 0.0F, 0.0F, 1.0F, 0.0F));
		indexs.push_back(0);
		indexs.push_back(pindex++);
		indexs.push_back(pindex++);
		...
		// 侧面
		v.push_back(Vertex(nx, height / 2.0F, nz, 0.0F, 1.0F, nx, 0.0F, nz));	// [1] pindex - 4
		v.push_back(Vertex(x, height / 2.0F, z, 1.0F, 0.0F, x, 0.0F, z));	    // [2] pindex - 3
		v.push_back(Vertex(x, -height / 2.0F, z, 1.0F, 0.0F, x, 0.0F, z));    // [4] pindex - 2
		v.push_back(Vertex(nx, -height / 2.0F, nz, 0.0F, 1.0F, nx, 0.0F, nz));  // [3] pindex - 1
		pindex += 4;
